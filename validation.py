...
def main(parser_data):
    ...
    test_dataset = Chromdataset_else(data_root, "test", data_transform["test"])
    # VOCdevkit -> VOC2012 -> ImageSets -> Main -> val.txt
    # test_dataset = VOCInstances(data_root, year="2012", txt_name="val.txt", transforms=data_transform["val"])
    ...
    model = OverlapMaskRCNN(backbone, num_classes=args.num_classes + 1)
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default="/home/xinyu.fan/pytorch_object_detection/mask_rcnn/data/chrom/", help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='/home/xinyu.fan/pytorch_object_detection/mask_rcnn/results_data/overlap_else/overlap_fusion1_focalloss/model_36.pth', type=str, help='training weights')

    # batch size(set to 1, don't change)
    parser.add_argument('--batch-size', default=1, type=int, metavar='N',
                        help='batch size when validation.')
    # 类别索引和类别名称对应关系
    parser.add_argument('--label-json-path', type=str, default="cocochrom_indices.json")
